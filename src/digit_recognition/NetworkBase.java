...
public class NetworkBase {
    ...
    public NetworkBase(int[] NETWORK_LAYER_SIZE) {
        ...
        for (int index = 0; index < NETWORK_SIZE; index++) {
            ...
            if (index > 0) {
                ...
                weights[index] = Utility.buildRandomArray(NETWORK_LAYER_SIZE[index], NETWORK_LAYER_SIZE[index - 1],
     ...
     * @param batch_size batch size used only in bigger sets ( should improve
     *                   accuracy )
     */
    public void train(TrainingSet set, int loops, int batch_size) {
        ...
        for (int index = 0; index < loops; index++) {
            ...
            for (int b = 0; b < batch_size; b++) {
                this.training(batch.getInput(b), batch.getOutput(b), MultiLayerPerceptron.LEARNING_RATE);
            }
        }
    }

    /**
     * Function for average squared difference between the estimated values and what
     * is estimated
     * 
    ...
    public double MeanSquaredErrorFunction(double[] input, double[] target) {
        ...
        for (int index = 0; index < target.length; index++) {
            error += (target[index] - output[NETWORK_SIZE - 1][index])
                    * (target[index] - output[NETWORK_SIZE - 1][index]);
        }
    ...
    /**
     * calculate a gradient that is needed in the calculation of the weights to be
     * used in the network
     * 
